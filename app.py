...
if mode == "企業分析":
    ...
    sales_current = st.number_input("今期売上高（百万円）", value=None, step=100.0, format="%.0f")
    sales_prev = st.number_input("前期売上高（百万円）", value=None, step=100.0, format="%.0f")
    op_profit = st.number_input("営業利益（百万円）", value=None, step=100.0, format="%.0f")
...
if mode == "決算レビュー":
    ...
    op_current = st.number_input("今期営業利益（百万円）", value=None, step=100.0, format="%.0f", key="op_current_2")
    op_prev = st.number_input("前期営業利益（百万円）", value=None, step=100.0, format="%.0f", key="op_prev_2")
    net_current = st.number_input("今期純利益（百万円）", value=None, step=100.0, format="%.0f", key="net_current_2")
    net_prev = st.number_input("前期純利益（百万円）", value=None, step=100.0, format="%.0f", key="net_prev_2")
...
【営業利益】今期 {op_current:,.0f} 百万円 ／ 前期 {op_prev:,.0f} 百万円（前年比：{op_yoy:.1f}%）
